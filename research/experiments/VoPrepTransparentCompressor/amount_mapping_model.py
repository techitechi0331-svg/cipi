...
def make_level_corpus() -> tuple[list[float], list[float], list[int]]:
    ...
    for phrase in range(10):
        ...
        for i in range(160):
            ...
            event.append(is_event)
    return slow, fast, event
...
def mapping_gr(mapping: str, amount: float, effective_db: float) -> float:
...
def main() -> None:
    ...
    for shift in shifts:
        ...
        full_eff = [soft_knee_gr(v, BASE_THRESHOLD_DB, BASE_RATIO) for v in effective]
        full_slow = [soft_knee_gr(v, BASE_THRESHOLD_DB, BASE_RATIO) for v in slow]
        full_extra = [max(0.0, a - b) for a, b in zip(full_eff, full_slow)]

        for mapping in mappings:
            for amount in amounts:
                gr = [mapping_gr(mapping, amount, v) for v in effective]
                slow_gr = [mapping_gr(mapping, amount, v) for v in slow]
                extra = [max(0.0, a - b) for a, b in zip(gr, slow_gr)]
                target = [amount * v for v in full_eff]
                target_extra = [amount * v for v in full_extra]

                lin_rmse = math.sqrt(sum((a-b)**2 for a,b in zip(gr,target))/len(gr))
                event_indices = [i for i,e in enumerate(events) if e]
                event_extra = [extra[i] for i in event_indices]
                event_target = [target_extra[i] for i in event_indices]
                event_rmse = math.sqrt(
                    sum((a-b)**2 for a,b in zip(event_extra,event_target))/max(len(event_extra),1)
                )
                finite = all(math.isfinite(v) for v in gr + extra)
                zero_null = max(gr) if amount == 0.0 else 0.0
                full_err = max(abs(a-b) for a,b in zip(gr,full_eff)) if amount == 1.0 else 0.0
                ...
                writer.writerow({
                    ...
                    "linearity_rmse_db": f"{lin_rmse:.9f}",
                    "zero_null_max_gr_db": f"{zero_null:.9f}",
                    "event_extra_mean_db": f"{sum(event_extra)/max(len(event_extra),1):.9f}",
                    "event_extra_linearity_rmse_db": f"{event_rmse:.9f}",
                    "full_scale_match_max_error_db": f"{full_err:.9f}",
