...
class VoPrepMono:
    ...
    def process(self, samples: list[float]) -> tuple[list[float], dict[str, list[float]]]:
        out = []
        traces = {
            "plosive_reduction_db": [],
            "plosive_probability": [],
            "macro_gain_db": [],
            "sibilance_reduction_db": [],
            "sibilance_probability": [],
        }
        for x in samples:
            y, pr, pp = self.plosive.process(x)
            y, mg, _, _, _ = self.macro.process(y)
            y, sr, sp = self.sibilance.process(y)
            out.append(y)
            traces["plosive_reduction_db"].append(pr)
            traces["plosive_probability"].append(pp)
            traces["macro_gain_db"].append(mg)
            traces["sibilance_reduction_db"].append(sr)
            traces["sibilance_probability"].append(sp)
        return out, traces
...
def source_plosive_positive_control(t: float) -> float:
    x = 0.04 * math.sin(2.0 * math.pi * 140.0 * t)
    if 0.8 <= t < 0.86:
        u = (t - 0.8) / 0.06
        env = math.sin(math.pi * u) ** 2
        x += env * (
            0.90 * math.sin(2.0 * math.pi * 45.0 * t)
            + 0.08 * math.sin(2.0 * math.pi * 120.0 * t)
        )
    return x


def source_sibilance_vowel(t: float) -> float:
    x = 0.0
    for harmonic in range(1, 35):
        frequency = 180.0 * harmonic
        if frequency >= 12000.0:
            break
        x += (0.12 / harmonic) * math.sin(2.0 * math.pi * frequency * t)
    return x


def source_sibilant_tones(t: float) -> float:
    frequencies = (5200.0, 6100.0, 7200.0, 8400.0, 9700.0, 11200.0)
    return sum(
        0.08 * math.sin(2.0 * math.pi * frequency * t + index * 0.7)
        for index, frequency in enumerate(frequencies)
    )


def source_event_envelope(t: float, start: float, end: float, attack: float, release: float) -> float:
    if t < start or t >= end:
        return 0.0
    if t < start + attack:
...
def make_case(name: str, sr: int) -> tuple[list[float], dict]:
    ...
    if name == "neutral_body":
        ...
        for i in range(n):
            y[i] = source_plosive_positive_control(i / sr)
        meta["event"] = (0.80, 1.05)
        meta["positive_control"] = "Vo.Prep Tests/PlosiveGuardTests.cpp scenario 0"

    elif name == "sibilance_on_body":
        for i in range(n):
            t = i / sr
            vowel = source_sibilance_vowel(t)
            env = source_event_envelope(t, 0.8, 0.95, 0.005, 0.015)
            y[i] = vowel * (1.0 - 0.9 * env) + source_sibilant_tones(t) * env
        meta["event"] = (0.80, 1.15)
        meta["positive_control"] = "Vo.Prep Tests/SibilanceGuardTests.cpp scenario 0"
...
def main() -> int:
    ...
    acceptance = {
        ...
        "plosive_guard_engages_ge_0_50db": metrics["plosive_min_guard_reduction_db"] >= 0.50,
        ...
        "sibilance_guard_engages_ge_0_30db": metrics["sibilance_min_guard_reduction_db"] >= 0.30,
    ...
    report = ["# Vo.Prep -> VoPriPro Integration Screen v2", "", f"Decision: **{'ELIGIBLE_FOR_REAL_VOCAL' if accepted else 'REVISE_OR_REJECT'}**", "", "## Aggregate metrics"]
