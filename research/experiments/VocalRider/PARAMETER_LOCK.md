> CIPI sync note: product implementation source of truth is `techitechi0331-svg/vocal_rider` main. Snapshot source HEAD: `c1a2a4f4d0bb408c9eeb37c077b735f59e744414`.
...
- lower guard limit: 0 dB (the guard may remove positive Rider boost, but may never create extra attenuation);
...
The guard constrains only positive requested ride before final Output gain application. Neutral (0 dB) and already-negative ride must pass unchanged, so Amount 0% cannot become a hidden limiter. It is shared by the VST3 processor and real-vocal validation renderer.
